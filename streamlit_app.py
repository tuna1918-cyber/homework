...
def create_sample_data():
    ...
    # 0.2의 확률로 큰 변동성(뉴스 등)을 시뮬레이션
    random_shocks = np.random.choice([1, 1, 1, 1, 3], 30) # 4:1 비율로 큰 변동성
    foreign_net = (np.random.randn(30) * 2000) * random_shocks
...
st.markdown(f"**기준일: {datetime.date.today().strftime('%Y-%m-%d')}** (이 앱은 가상 데이터로 실행되고 있습니다.)")
...
# KOSPI와 누적 순매수 비교
# 두 데이터의 스케일(단위)이 다르므로 2개의 차트로 분리
