네, 죄송합니다. 코드에 주석으로 출처를 표기하는 과정에서 ``와 같은 AI 내부 마커가 실수로 포함되어 SyntaxError가 발생했습니다.

이 마커들과 불필요한 주석을 모두 제거한 수정된 전체 코드를 다시 제공해 드립니다.

1. app.py (오류 수정된 전체 코드)
이 코드를 app.py 파일에 붙여넣으시면 SyntaxError 없이 정상적으로 실행됩니다.
